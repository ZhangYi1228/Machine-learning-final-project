...
import events as e
from .callbacks import (transform,
                        state_to_features,
                        fname,
                        FILENAME)
...
def setup_training(self):
    ...
    # Storage of states for feature extraction function learning.
    if not self.dr_override:
        self.state_history = deque(maxlen=DR_HISTORY_SIZE)

    # Set initial epsilon/tau.
    ...
    if old_game_state:
        ...
        if islethal_old:
            ...
            if self_action == 'BOMB':
                ...
                elif crates_act_old == BOMB:
                    events.append(BOMBED_CRATE)
                
                '''
                if others_act_old == BOMB or (crates_act_old == BOMB and others_act_old == WAIT):
                    events.append(BOMBED_GOAL)
                else:
                    events.append(BAD_BOMB)
            elif self_action == 'WAIT':
                if others_act_old == WAIT and coins_act_old == WAIT and crates_act_old == WAIT:
            ...
            if self_action == 'BOMB':
                ...
                if check_action(others_act_old,self_action):
                    if others_acq_old == 1:
                        events.append(BOMBED_OTHERS)
                    else: events.append(BOMBED_OBSTACLES)
                # coins
                elif check_action(coins_act_old,self_action):
                    events.append(BOMBED_OBSTACLES)
                # crates
                elif check_action(crates_act_old,self_action):
                    if crates_acq_old == 1:
                        events.append(BOMBED_CRATE)
                    else:
                        events.append(BOMBED_OBSTACLES)
                # not recommend
                    ...
                    if others_acq_old == 0:
                        events.append(MISSED_OTHERS)
                    elif crates_acq_old == 0:
                        events.append(MISSED_CRATE)
                    else:
                        events.append(MISSED_OBSTACLES)
            # movements and wait
            else:
                # others
                if others_act_old != BOMB:
                    if check_action(others_act_old,self_action):
                        if others_act_old == WAIT:
                            if others_acq_old == 1:
                                ...
                                events.append(PASS_STEP)
                        else: events.append(CLOSER_TO_OTHERS)
                    else:
                        if others_act_old == WAIT:
                            if others_acq_old == 1:
                                ...
                                events.append(PASS_STEP)
                        else: events.append(FURTHER_FROM_OTHERS)
                # coins
                elif coins_act_old != BOMB:
                    if check_action(coins_act_old,self_action):
                        events.append(CLOSER_TO_COIN)
                    else:
                        events.append(FURTHER_FROM_COIN)
                ...
                # no targets, the arena is blank
                if have_targets == 0:
                    if self_action == 'WAIT':
                        events.append(WAITED_UNNECESSARILY)
                    else : events.append(WAITED_NECESSARILY)
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    # Store the game state for learning of feature extraction function.
        ...
        for state, action, next_state, reward in batch:
            ...
            if state is not None:
                ...
                if PRIO_EXP_REPLAY and self.model_is_fitted:
                    ...
                    X_tmp = np.array(X[action_idx][-1]).reshape(1, -1)
                    target = np.array(y[action_idx][-1])
