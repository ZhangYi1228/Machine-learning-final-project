...
# ---------------- Parameters ----------------
FILENAME = "Yi_Zhang-Changjing_Hu_agent"        # Base filename of model (excl. extensions).
ACT_STRATEGY = 'eps-greedy'     # Options: 'softmax', 'eps-greedy'
VALID_ACTIONS = False  # Enable/disable filtering of invalid actions.
# --------------------------------------------
...
def act(self, game_state: dict) -> str:
    ...
    if VALID_ACTIONS:
        mask, valid_actions = Action_Filter(game_state, filter_level='full')
    else:
...
def state_to_features(game_state: dict) -> np.array:
    ...
    lethal = is_lethal(x, y, arena, bombs, explosion_map)

    # ---- ESCAPE ----
    # Direction towards the closest escape from imminent danger.
    escape_direction = escape_dir(x, y, arena, bombs, others, explosion_map)
    
    # ---- OTHERS ----
    # Direction towards the best offensive tile against other agents.
    others_direction, others_reached = others_dir(x, y, 5, arena, bombs, others, explosion_map)

    # ---- COINS ----
    # Direction towards the closest reachable coin.
    coin_direction = coins_dir(x, y, coins, arena, bombs, others, explosion_map)

    # ---- CRATES ----
    # Direction towards the best offensive tile for destroying crates.
    crates_direction, crates_reached = crates_dir(x, y, 10, arena, bombs, others,explosion_map)
...
def is_lethal(x: int, y: int, arena: np.array, bombs: list,explosion_map) -> bool:
    """
...
def escape_dir(x: int, y: int, arena: np.array, bombs: list, others: list,explosion_map) -> np.array:
    """
    ...
    if bombs:
        ...
        while not q.empty():             
            (ix, iy), parent = q.get()              
            graph[(ix, iy)] = parent
            if not is_lethal(ix, iy, arena, bombs,explosion_map):
                escapable = True
                break
...
def crates_dir(x: int, y: int, n: int, arena: np.array, bombs: list, others: list,explosion_map) -> (np.array, bool):
    """
    ...
    if candidates:
        ...
        if len(s) > 1:
            ...
            if not is_lethal(nx, ny, arena, bombs,explosion_map):
...
def coins_dir(x: int, y: int, coins: list, arena: np.array, bombs: list, others: list,explosion_map) -> np.array:
    """
...
def others_dir(x: int, y: int, n: int, arena: np.array, bombs: list, others: list,explosion_map) -> (np.array, bool):
    """
...
def Action_Filter(game_state: dict, filter_level: str='basic'):
    """
    ...
    if filter_level == 'full':
        ...
        for i, (ix, iy) in enumerate(directions):
            if not has_object(ix, iy, arena, 'wall'):
                lethal_status[i] = int(is_lethal(ix, iy, arena, bombs,bomb_map))
            else:
                lethal_status[i] = -1
